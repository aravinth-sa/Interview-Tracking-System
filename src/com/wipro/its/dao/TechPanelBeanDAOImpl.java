...
@Repository
public class TechPanelBeanDAOImpl implements TechPanelBeanDAO{
	@Autowired
	 SessionFactory sessionFactory;

	@SuppressWarnings("unchecked")
	@Override
	public ArrayList<CandidateBean> findAll(String techID, Date interviewDate) {
			...
			Criteria cr = session.createCriteria(InterviewScheduleBean.class);
				cr.add(Restrictions.eq("techID", techID));
				
				 
				cr.add(Restrictions.eq("interviewDate", interviewDate));
				
				cr.add(Restrictions.eq("shareResult", 0));

				
				ArrayList<InterviewScheduleBean> d= (ArrayList<InterviewScheduleBean>) cr.list();
				Iterator<InterviewScheduleBean> it=d.iterator();
				while(it.hasNext()){
					InterviewScheduleBean is=it.next();
					System.out.println(is);
					String candidateID=is.getCandidateID();
					CandidateBean cb=(CandidateBean) session.get( CandidateBean.class,candidateID);
	...
	@Override
	public boolean submitRatings(String interviewID, String techID,
			float emptechRating) {
		Session session = sessionFactory.openSession();
